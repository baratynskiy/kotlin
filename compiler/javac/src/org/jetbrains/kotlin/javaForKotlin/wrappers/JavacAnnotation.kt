...
open class JavacAnnotation(val annotationMirror: AnnotationMirror) : JavaElement, JavaAnnotation {

    override val arguments: Collection<JavaAnnotationArgument>
        get() = annotationMirror.elementValues
                .map { JavacAnnotationArgument.create(it.value.value, Name.identifier(it.key.simpleName.toString())) }

    override val classId: ClassId?
        get() = (annotationMirror.annotationType.asElement() as? TypeElement)?.computeClassId()

    override fun resolve() = JavacClass(annotationMirror.annotationType.asElement() as TypeElement)
