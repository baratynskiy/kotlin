...
object JavaWithKotlinCompiler {
    ...
    fun findType(name: String): TypeElement? = symbols.classes
            .filter { (_,v)  -> v.fullname.toString() == name }
            .values.firstOrNull()

    fun findPackage(name: String): PackageElement? = symbols.packages
            .filter { (_,v)  -> v.fullname.toString() == name }
            .values.firstOrNull()

    fun getSubpackages(name: String) = symbols.packages
